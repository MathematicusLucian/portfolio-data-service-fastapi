# lambda-portfolio-data-service Portfolio Read
...
``sam build --use-container && sam local start-api -n .env.json --skip-pull-image -p 8080``


Debugging:

``sam build --use-container && sam local start-api -n .env.json --skip-pull-image -d -5858``
...
```bash
lambda-portfolio-data-service$ sam build
```
...
```bash
lambda-portfolio-data-service$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-portfolio-data-service$ sam local start-api
lambda-portfolio-data-service$ curl http://localhost:3000/
```
...
```bash
lambda-portfolio-data-service$ sam logs -n HelloWorldFunction --stack-name "lambda-portfolio-data-service" --tail
```
...
```bash
lambda-portfolio-data-service$ pip install pytest pytest-mock --user
lambda-portfolio-data-service$ python -m pytest tests/ -v
```
...
```bash
sam delete --stack-name "lambda-portfolio-data-service"
```
